...
describe('UserRepository', () => {
  ...
  describe('signUp', () => {
    ...
    it('successfully signs up the user', async () => {
      jest.spyOn(userRepository, 'save').mockImplementation(() => Promise.resolve());
    ...
    it('throws a conflict exception as username already exists', async () => {
      jest.spyOn(userRepository, 'save').mockImplementation(() => Promise.reject({ code: '23505' }));
      await expect(userRepository.signUp(mockCredentialsDto)).rejects.toThrow(ConflictException);
    });

    it('throws a InternalServerErrorException as unknown reason', async () => {
      jest.spyOn(userRepository, 'save').mockImplementation(() => Promise.reject({ code: '123123' }));
